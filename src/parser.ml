...
let flush_line reader =
  read_string reader |> Deferred.Result.map ~f:ignore

let read_int reader =
  read_string reader |> Deferred.Result.map ~f:int_of_string
...
let read_bulk ~len reader =
  ...
  | `Ok ->
    Deferred.Result.return (Bytes.to_string s)
